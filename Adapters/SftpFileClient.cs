...
public sealed class SftpFileClient : IFileClient
{
    ...
    /// <summary>
    /// Fired on first connection to a host when TOFU trust is applied.
    /// Parameters: (host, fingerprint).
    /// AppBridge wires this to post a notification to the frontend.
    /// </summary>
    public Action<string, string>? HostKeyFirstTrust { get; set; }
    ...
    private void OnHostKeyReceived(object? sender, Renci.SshNet.Common.HostKeyEventArgs e)
    {
        ...
        if (stored is null)
        {
            ...
            // Notify via callback (e.g., to show fingerprint in UI)
            HostKeyFirstTrust?.Invoke(_profile!.Host, fingerprint);
